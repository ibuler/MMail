...
#username = '172658139@qq.com' ##已经不用了,直接从文件中调用
#password = 'rgetcf'
subject = u'Linux优秀文章大放送'  #邮件主题

smtpHost = 'smtp.qq.com'
smtpPort = 465
toMails = []                      ##将来发送的邮件列表
f = open('mail_list.txt')         #打开文件读取邮件列表
for mail in f:
    toMails.append(mail)
f.close()
...
##初始化邮件
encoding = 'utf-8'
mail = MIMEText(body.encode(encoding), 'html', encoding)  #邮件内容转化为html格式
...
while True:
    ...
    for username, password in auth.items(): 
        ...
        smtp.ehlo()
        try:
            print '#'*20,'%s:%s' % (username, password), '#'*20
